                    ...
                    <table class="table">
                      <tbody>
                        ...
                        while ($row2 = $query2->fetch_object()) {
                          $query3 = $conn->query("SELECT * FROM responses WHERE cfm_id=$row->id and cfsm_id=$row2->id and msme_id=$msme_id and value = 1");
                              ...
                              <div class="float-end">
                                <div class="form-check form-check-inline mb-3">
                                  <input class="form-check-input"
                                    ...
                                    name="options<?= $row2->id ?>" id="yesradio<?= $row2->id ?>"
                                    <?= isset($get_query3->value) && $get_query3->value == 1 ? 'checked' : '' ?> required />
                                  <label class="form-check-label" for="yesradio<?= $row2->id ?>">Yes</label>
                                </div>
                                <div class="form-check form-check-inline mb-3">
                                  <input class="form-check-input"
                                    onclick="return cfa(<?= $row->id ?>,<?= $row2->id ?>,<?= $msme_id ?>,0)" type="radio"
                                    name="options<?= $row2->id ?>" id="noradio<?= $row2->id ?>" 
                                    <?= isset($get_query3->value) && $get_query3->value == 1 ? '' : 'checked' ?> />
                                  <label class="form-check-label" for="noradio<?= $row2->id ?>">No</label>
                                </div>
                              </div>
                            </td>
                            <td id="colorIndicator<?= $row2->id ?>"
                              class="<?= $conn->query("SELECT * FROM responses WHERE cfm_id=$row->id and cfsm_id=$row2->id and msme_id=$msme_id")->num_rows ? ($query3->num_rows == 1 ? 'bg-success' : 'bg-danger') : 'bg-secondary' ?>"
                              width="5%"></td>
                          </tr>
                          <?php
                        }
    ...
    function cfa(cfm_id, cfsm_id, msme_id, value) {
      $.ajax({
        ...
        success: function (response) {
          console.log("message");
          if (value == 1) {
            $('#colorIndicator' + cfsm_id).addClass('bg-success').removeClass('bg-danger');
          } else {
            $('#colorIndicator' + cfsm_id).addClass('bg-danger').removeClass('bg-success');
          }
        },
      });
    }
